...
class Staffer {
  ...
  static async updateStafferPassword(db, stafferData) {
    return new Promise((resolve, reject) => {
      const passwordEncrypted = bcrypt.hashSync(stafferData.newPassword, 10);
      const query =
        "SELECT password FROM staffer WHERE idStaffer = ? AND password = ?";
      db.query(
        query,
        [stafferData.id, stafferData.oldPassword],
        (err, results) => {
          if (err) {
            console.error("Errore durante la query:", err);
            return reject("Errore interno del server");
